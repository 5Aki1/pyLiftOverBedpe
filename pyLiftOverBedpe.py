...
def lift_bedpe(input_bedpe, output_bedpe, chain_file):
    converter = LiftOver(chain_file)
    ...
    with open(input_bedpe, 'r') as infile, open(output_bedpe, 'w') as outfile:
        for line in infile:
            ...
            if lifted_start1 and lifted_end1 and lifted_start2 and lifted_end2:
                outfile.write(
                    "{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                        lifted_start1[0][0], lifted_start1[0][1], lifted_end1[0][1],
                        lifted_start2[0][0], lifted_start2[0][1], lifted_end2[0][1],
                        "\t".join(extra_fields)
                    )
                )
            else:
...
def main():
    ...
    lift_bedpe(args.input, args.output, args.chain_file)
